...
class Needell_SSNMF(SSNMF_Application):
    ...
    def __init__(self, X, Y, split_train_test=True, train_indices=None, test_indices=None, *args, **kwargs):
        '''
        Initializes Haddock_SSNMF. If split_train_test is False, must provide train_indices, test_indices array.
    ...
    def fulldata_validate(self, param_vals, **kwargs):
        ...
        SSNMF on training set to compute Fulldata training accuracy. Use mask that hides testing data (testing mask).
        Then SSNMF on testing set to compute testing accuracy.
        ...
        train_score = self.get_accuracy(full_model, self.train_indices)
        test_score =  self.get_accuracy(full_model, self.test_indices) 
    ...
    def get_best_fulldata_model(self, **kwargs):
        '''
        Train Pypi_SSNMF with no mask (no data hidden) using the parameters that produce best training accuracy,
