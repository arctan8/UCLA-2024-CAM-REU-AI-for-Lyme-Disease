...
class Haddock_SSNMF(SSNMF_Application):
    ...
    def fulldata_validate(self, param_vals, **kwargs):
        ...
        '''
        N = kwargs.get('N', 1000)
        full_model = Pypi_SSNMF(X=self.X.T, Y=self.Y.T, W=self.W.T, k=param_vals['k'], lam=param_vals['lambda'], random_state=param_vals['random_state'], modelNum=3)

        full_model.mult(numiters=N)
        # Here X_tst_err is the same as get_Xreconerr
        score, X_tst_err =  self.get_accuracy(full_model, self.X.T, self.Y.T, S=full_model.S, W=self.W.T) 
    ...
    def get_best_fulldata_model(self, **kwargs):
        ...
        self.best_fulldata_model = Pypi_SSNMF(X=self.X.T, Y=self.Y.T, W=self.W.T, k=param_vals['k'], lam=param_vals['lambda'], random_state=param_vals['random_state'], modelNum=3)
